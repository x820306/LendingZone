...
router.get('/search/:keyword?/:category?/:messenger?/:action?/:director?/:lbound?/:ubound?/:page?',library.loginFormChecker,library.newMsgChecker,library.usrNameGenerator, function (req, res) {
	if((typeof(req.query.keyword) === 'string')&&(typeof(req.query.category) === 'string')&&(typeof(req.query.messenger) === 'string')&&(typeof(req.query.action) === 'string')&&(typeof(req.query.director) === 'string')&&(typeof(req.query.lbound) === 'string')&&(typeof(req.query.ubound) === 'string')&&(typeof(req.query.page) === 'string')){
		...
		if(!isNaN(targetPage)){
			var objFlag=false;
			var resArrays=[];
			var action=decodeURIComponent(library.replacer(req.query.action,false));
			var category=decodeURIComponent(library.replacer(req.query.category,false));
			var messenger=decodeURIComponent(library.replacer(req.query.messenger,false));
			var director=decodeURIComponent(library.replacer(req.query.director,false));
			var lbound=decodeURIComponent(library.replacer(req.query.lbound,false));
			var ubound=decodeURIComponent(library.replacer(req.query.ubound,false));
			var keyword=decodeURIComponent(library.replacer(req.query.keyword,true));
			var pageNum=0
						...
						if(targetPage>1){
							...
							req.session.save(function(err){
								if(err){
									...
									res.render('search',{maxAge:req.session.cookie.maxAge,lgfJSON:req.loginFormJson,newlrmNum:req.newlrmNumber,newlsmNum:req.newlsmNumber,userName:req.auRst,keywordDefault:keyword,messengerDefault:messenger,actionDefault:action,categoryDefault:category,directorDefault:director,lboundDefault:lbound,uboundDefault:ubound,jsonArray:resArrays,totalResultNum:totalResultNumber,pageNumber:pageNum,targetPageNumber:targetPage,oflg:objFlag,pflg:plusFlag,kpr:keeper,dvr:divider});
								}
							});
						}
					}else{
								...
								for(i=borrows.length-1;i>-1;i--){
									if(req.auRst===null){
										...
										for(j=0;j<borrows[i].Message.length;j++){
											if((borrows[i].Message[j].CreatedBy.equals(req.user._id))&&(borrows[i].Message[j].Type==="toLend")){
												...
												break;
											}else if((borrows[i].Message[j].SendTo.equals(req.user._id))&&(borrows[i].Message[j].Type==="toBorrow")){
												if(borrows[i].Message[j].Status==="NotConfirmed"){
										...
										for(i=0;i<resArrays.length;i++){
											...
											if(req.auRst!==null){
												...
												for(j=0;j<resArrays[i].Likes.length;j++){
													if(resArrays[i].Likes[j].equals(req.user._id)){
...
router.get('/story/:id?',library.loginFormChecker,library.newMsgChecker,library.usrNameGenerator, function (req, res){
							...
							if(req.auRst===null){
								req.session.save(function(err){
									if(err){
										...
										res.render('story',{maxAge:req.session.cookie.maxAge,lgfJSON:req.loginFormJson,newlrmNum:req.newlrmNumber,newlsmNum:req.newlsmNumber,userName:req.auRst,ifSelf:ifSelfValue,ifLiked:ifLikedValue,json:borrow,jsonMessage:null,jsonBorrowMessage:null,jsonLend:null,MoneyInBankAccountValue:0,MoneyLended:moneyLendedJson,hlfJSON:hendLendFormJson,cfJSON:confirmFormJson});
									}
								});
							}else{
							...
							if (err) {
								...
								req.session.save(function(err){
									if(err){
										...
										res.render('lend',{maxAge:req.session.cookie.maxAge,lgfJSON:req.loginFormJson,newlrmNum:req.newlrmNumber,newlsmNum:req.newlsmNumber,userName:req.user.Username,MoneyInBankAccountValue:bankaccount.MoneyInBankAccount,MoneyLended:moneyLendedJson,jsonLend:lend,lfJSON:lendFormJson,scr:library.serviceChargeRate});
									}
								});
							}
						});
...
router.get('/lenderTransactionRecord/:oneid?/:filter?/:messenger?/:classor?/:sorter?/:director?/:lbound?/:ubound?/:page?',library.loginFormChecker, library.ensureAuthenticated,library.newMsgChecker, function (req, res) {
	if((typeof(req.query.oneid) === 'string')&&(typeof(req.query.filter) === 'string')&&(typeof(req.query.messenger) === 'string')&&(typeof(req.query.classor) === 'string')&&(typeof(req.query.sorter) === 'string')&&(typeof(req.query.director) === 'string')&&(typeof(req.query.lbound) === 'string')&&(typeof(req.query.ubound) === 'string')&&(typeof(req.query.page) === 'string')){
		...
		if(!isNaN(targetPage)){
			...
			var objFlag=false;
			var resArrays=[];
			var oneid=decodeURIComponent(library.replacer(req.query.oneid,true));
			var sorter=decodeURIComponent(library.replacer(req.query.sorter,false));
			var filter=decodeURIComponent(library.replacer(req.query.filter,false));
			var director=decodeURIComponent(library.replacer(req.query.director,false));
			var lbound=decodeURIComponent(library.replacer(req.query.lbound,false));
			var ubound=decodeURIComponent(library.replacer(req.query.ubound,false));
			var messenger=decodeURIComponent(library.replacer(req.query.messenger,false));
			var classor=decodeURIComponent(library.replacer(req.query.classor,false));
			var pageNum=0
			var totalResultNumber=0;
			var divider=10;
							...
							if(err){
								...
								if(totalResultNumber===0){
									...
									for(p=0;p<totalResultNumber;p++){
										...
										dataPack.v1+=transactions[p].PrincipalNotReturn;
										dataPack.v2+=transactions[p].InterestInFuture;
										dataPack.v3+=transactions[p].MoneyFuture;
										dataPack.v4+=transactions[p].InterestInFutureMonth;
										dataPack.v5+=transactions[p].InterestInFutureMoneyMonth;
										dataPack.v6+=transactions[p].PrincipalReturn;
										dataPack.v7+=transactions[p].Interest;
											...
											if (err){
												...
												if(transactions.length>0){
													for(i=0;i<transactions.length;i++){
														library.transactionProcessor(transactions[i],false);
														if(transactions[i].CreatedFrom.Type==='toBorrow'){
															dataPack.autoLendCumulated+=transactions[i].PrincipalNotReturn;
														}else if(transactions[i].CreatedFrom.Type==='toLend'){
															dataPack.hendLendCumulated+=transactions[i].PrincipalNotReturn;
														}
													}
													dataPack.moneyLendedCumulated=dataPack.hendLendCumulated+dataPack.autoLendCumulated;
												}
														...
														if(!bankaccount){
															...
															Lends.findOne({"CreatedBy": req.user._id}).exec( function (err, lend){
																if (err) {
																	...
																	if(lend){
																		dataPack.moneyLeftToAutoLend=lend.MaxMoneyToLend-dataPack.autoLendCumulated;
																		if(dataPack.moneyLeftToAutoLend<=0){
																			dataPack.moneyLeftToAutoLend=0;
																		}
																	}
																	dataPack.moneyLeftToHendLend=bankaccount.MoneyInBankAccount-dataPack.moneyLeftToAutoLend;
																	if(dataPack.moneyLeftToHendLend<=0){
																		dataPack.moneyLeftToHendLend=0;
																	}
																	dataPack.maxSettingAutoLend=bankaccount.MoneyInBankAccount+dataPack.autoLendCumulated;
																	req.session.save(function(err){
																		if(err){
																			console.log(err);
																			res.redirect('/message?content='+encodeURIComponent('錯誤!'));
...
router.get('/lenderReturnRecord/:oneid?/:id?/:messenger?/:classor?/:sorter?/:director?/:lbound?/:ubound?/:page?',library.loginFormChecker, library.ensureAuthenticated, library.newMsgChecker,function (req, res) {
	if((typeof(req.query.oneid) === 'string')&&(typeof(req.query.id) === 'string')&&(typeof(req.query.messenger) === 'string')&&(typeof(req.query.classor) === 'string')&&(typeof(req.query.sorter) === 'string')&&(typeof(req.query.director) === 'string')&&(typeof(req.query.lbound) === 'string')&&(typeof(req.query.ubound) === 'string')&&(typeof(req.query.page) === 'string')){
		...
		if(!isNaN(targetPage)){
			...
			var resArrays=[];
			var oneid=decodeURIComponent(library.replacer(req.query.oneid,true));
			var id=decodeURIComponent(library.replacer(req.query.id,false));
			var sorter=decodeURIComponent(library.replacer(req.query.sorter,false));
			var director=decodeURIComponent(library.replacer(req.query.director,false));
			var lbound=decodeURIComponent(library.replacer(req.query.lbound,false));
			var ubound=decodeURIComponent(library.replacer(req.query.ubound,false));
...
router.get('/lendsList/:oneid?/:classOne?/:classTwo?/:sorter?/:director?/:lbound?/:ubound?/:page?',library.loginFormChecker, library.ensureAuthenticated, library.newMsgChecker,function (req, res) {
	if((typeof(req.query.oneid) === 'string')&&(typeof(req.query.classOne) === 'string')&&(typeof(req.query.classTwo) === 'string')&&(typeof(req.query.sorter) === 'string')&&(typeof(req.query.director) === 'string')&&(typeof(req.query.lbound) === 'string')&&(typeof(req.query.ubound) === 'string')&&(typeof(req.query.page) === 'string')){
		...
		if(!isNaN(targetPage)){
			...
			var resArrays=[];
			var oneid=decodeURIComponent(library.replacer(req.query.oneid,true));
			var classOne=decodeURIComponent(library.replacer(req.query.classOne,false));
			var classTwo=decodeURIComponent(library.replacer(req.query.classTwo,false));
			var sorter=decodeURIComponent(library.replacer(req.query.sorter,false));
			var director=decodeURIComponent(library.replacer(req.query.director,false));
			var lbound=decodeURIComponent(library.replacer(req.query.lbound,false));
			var ubound=decodeURIComponent(library.replacer(req.query.ubound,false));
				...
				if (err) {
					...
					if(lends.length===0){
						if(targetPage>1){
							...
							res.render('lendsList',{maxAge:req.session.cookie.maxAge,lgfJSON:req.loginFormJson,newlrmNum:req.newlrmNumber,newlsmNum:req.newlsmNumber,userName:req.user.Username,oneidDefault:oneid,classOneDefault:classOne,classTwoDefault:classTwo,sorterDefault:sorter,jsonLends:resArrays,totalResultNum:totalResultNumber,pageNumber:pageNum,targetPageNumber:targetPage,directorDefault:director,lboundDefault:lbound,uboundDefault:ubound,scr:library.serviceChargeRate,oflg:objFlag,pflg:plusFlag,kpr:keeper,dvr:divider});
						}
					}else{			
...
router.get('/lenderSendMessages/:msgKeyword?/:filter?/:classor?/:sorter?/:director?/:lbound?/:ubound?/:page?',library.loginFormChecker, library.ensureAuthenticated,library.newMsgChecker, function (req, res) {
	if((typeof(req.query.msgKeyword) === 'string')&&(typeof(req.query.filter) === 'string')&&(typeof(req.query.classor) === 'string')&&(typeof(req.query.sorter) === 'string')&&(typeof(req.query.director) === 'string')&&(typeof(req.query.lbound) === 'string')&&(typeof(req.query.ubound) === 'string')&&(typeof(req.query.page) === 'string')){
		...
		if(!isNaN(targetPage)){
			...
			var objFlag=false;
			var resArrays=[];
			var msgKeyword=decodeURIComponent(library.replacer(req.query.msgKeyword,true));
			var filter=decodeURIComponent(library.replacer(req.query.filter,false));
			var sorter=decodeURIComponent(library.replacer(req.query.sorter,false));
			var director=decodeURIComponent(library.replacer(req.query.director,false));
			var lbound=decodeURIComponent(library.replacer(req.query.lbound,false));
...
router.get('/income',library.loginFormChecker, library.ensureAuthenticated, library.newMsgChecker,function (req, res) {
	...
	Transactions.find({$and:[{Lender:req.user._id}]}).populate('Return').populate('Borrower','Level').exec( function (err, transactions, count){
		if (err) {
			...
			totalResultNumber=transactions.length;
			if(totalResultNumber<=0){
				res.render('income',{maxAge:req.session.cookie.maxAge,lgfJSON:req.loginFormJson,newlrmNum:req.newlrmNumber,newlsmNum:req.newlsmNumber,userName:req.user.Username,totalResultNum:totalResultNumber,monPriNRNow:monthPrincipalNotReturnNow,monRevNow:monthRevenueNow,monRoiNow:monthRoiNow,monPriNow:monthPrincipalNow,monRevPriNow:monthRevenuePrincipalNow,yrPriNR:yearPrincipalNotReturn,yrRoi:yearRoi,yrRev:yearRevenue,yrPri:yearPrincipal,yrRevPri:yearRevenuePrincipal,yrHistoryPriNR:yearHistoryPrincipalNotReturn,yrHistoryRoi:yearHistoryRoi,yrHistoryRev:yearHistoryRevenue,yrHistoryPri:yearHistoryPrincipal,yrHistoryRevPri:yearHistoryRevenuePrincipal,data01:data1,data02:data2,data03:data3,data04:data4,data05:data5,data06:data6,data07:data7,data08:data8,data09:data9,data010:data10,data011:data11,data012:data12});
			}else{
				...
				for(i=0;i<totalResultNumber;i++){
					if((transactions[i].Borrower.Level>=1)&&(transactions[i].Borrower.Level<5)){
				...
				dataPack.d7=dataPack.d7.toFixed(0);
				dataPack.d8=dataPack.d8.toFixed(0);
				dataPack.d9=dataPack.d9.toFixed(0);
				dataPack.d10=dataPack.d10.toFixed(0);
				dataPack.d11=dataPack.d11.toFixed(0);
				...
				for(i=0;i<totalResultNumber;i++){
					if(transactions[i].PrincipalNotReturn<=0){
						data12Array[0].value+=1;
						dataPack.d12+=1;
					}else{
						if(transactions[i].InsuranceFeePaid<=0){
							data12Array[1].value+=1;
							dataPack.d13+=1;
						}else{
							data12Array[2].value+=1;
							dataPack.d14+=1;
						}
					}
				}
				dataPack.d12=dataPack.d12.toFixed(0);
				dataPack.d13=dataPack.d13.toFixed(0);
				dataPack.d14=dataPack.d14.toFixed(0);
				
				totalTemp=0;
				for(i=0;i<data12Array.length;i++){
					totalTemp+=data12Array[i].value;
				}

				for(i=0;i<data12Array.length;i++){
					data12Array[i].value=data12Array[i].value/totalTemp*100;
				}
				data12.array=data12Array;
				
				res.render('income',{maxAge:req.session.cookie.maxAge,lgfJSON:req.loginFormJson,newlrmNum:req.newlrmNumber,newlsmNum:req.newlsmNumber,userName:req.user.Username,dpk:dataPack,totalResultNum:totalResultNumber,monPriNRNow:monthPrincipalNotReturnNow,monRevNow:monthRevenueNow,monRoiNow:monthRoiNow,monPriNow:monthPrincipalNow,monRevPriNow:monthRevenuePrincipalNow,yrPriNR:yearPrincipalNotReturn,yrRoi:yearRoi,yrRev:yearRevenue,yrPri:yearPrincipal,yrRevPri:yearRevenuePrincipal,yrHistoryPriNR:yearHistoryPrincipalNotReturn,yrHistoryRoi:yearHistoryRoi,yrHistoryRev:yearHistoryRevenue,yrHistoryPri:yearHistoryPrincipal,yrHistoryRevPri:yearHistoryRevenuePrincipal,data01:data1,data02:data2,data03:data3,data04:data4,data05:data5,data06:data6,data07:data7,data08:data8,data09:data9,data010:data10,data011:data11,data012:data12});
			}
		}
	});
});
